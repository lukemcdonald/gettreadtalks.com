...
const IntroTagline = ({ children, className }) => (
  <div className={classNames('mt-2 text-2xl font-light text-gray-400', className)}>{children}</div>
)
...
class Intro extends Component {
  ...
  render() {
    const { align, bgGradient, children, className, image, fullscreen } = this.props

    const imageSrc = image?.childImageSharp?.gatsbyImageData || image?.localFiles?.[0]?.childImageSharp?.gatsbyImageData
